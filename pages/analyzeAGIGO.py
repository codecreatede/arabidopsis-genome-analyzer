...
st.markdown('<div style="text-align: justify;">   this provides the Go information for the AGI ids presented in a file and presents them as tuples with the AGI at index 0 and the GO at index 1. This takes the gene_association.tair as the association file.</div>',
...
idsfile and associationfile and st.button("Analyze AGIGO"):
    ...
    with open(os.path.abspath(os.path.join(os.getcwd(), ids_file)), "r") as ids:
        ...
        analyzed = [j for i in final_ids for j in ([(i,j) for i,j in
zip(AGI,GO)]) if j[0] == i]
        st.write(analyzed)
